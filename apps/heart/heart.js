...
function doread(){
  ...
  if (det(v)) {
    ...
    if (bpm > 0 && bpm < 200) {
      bpm = bf(bpm);
      g.clearRect(10,5,100,25);
      g.setColor(3);
      g.drawString("BPM: "+bpm,10,5,true);
    }
    lastPeak=peakTime;
  }
  g.setColor(0).fillRect(x,75,x+1,175);
  g.setColor(3).setPixel(x,125).setPixel(x+1,125);
  g.setColor(2).fillRect(x,lasty,x+1,v);
  g.flip();
  lasty=v;
  x+=2;
  if (x>=176) x = 0;
...
function showBPM(){
  ...
  g.clearRect(10,35,100,60);
  g.setColor(1).drawString("BPM: "+bpm,10,35,true);
  g.flip();
}
...
function stopMeasure() {
  if(interval) {
    ...
    HRS.disable();
    g.setColor(1).drawString("PAUSED",88,20,true).flip();
    g.restoreColors();
  }
}
