...
/* 
stripped down version of fanoush's driver for ST7301 - supports only 2 bpp but allows more than 
four colours by allowing palette colours to be redefined between flips. 
*/
var SPI2 = (function(){
...
function ST7301(){
  ...
  var colors = [0x00,0x0F,0x0C,0x3f]
  var pal = new Uint8Array(toFlatBuffer(colors));
  
  g.restoreColors= function(){
    for (var i=0; i<colors.length; i++) pal[i]=colors[i];
  }

  g.defColor = function(n,c){
    if (n<0 || c<0 || n>3 || c>63) return;
    pal[n]=c;
  }
